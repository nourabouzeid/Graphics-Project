...
class Menustate : public our::State
{
    ...
    void onInitialize() override
    {
        ...
        menuMaterial->texture = our::texture_utils::loadImage("assets/textures/menu4.jpg");
        // Initially, the menu material will be black, then it will fade in
        menuMaterial->tint = glm::vec4(1.f, 1.0f, 1.0f, 1.0f); // Dark gray
        ...
        highlightMaterial->tint = glm::vec4(0.0f, 0.0f, 0.0f, 0.5f); // Semi-transparent white
        ...
        buttons[0].position = {550.0f, 510.0f};
        buttons[0].size = {180.0f, 60.0f};
        buttons[0].action = [this]()
        { this->getApp()->changeState("play"); };

        buttons[1].position = {550.0f, 615.0f};
        buttons[1].size = {180.0f, 60.0f};
    ...
    void onDraw(double deltaTime) override
    {
        ...
        for (auto &button : buttons)
        {
            if (button.isInside(mousePosition))
            {
                // Make button fully white when hovered
                highlightMaterial->tint = glm::vec4(0.0f, 0.0f, 0.0f, 0.3f);
            }
            else
            {
                // Slightly transparent white when not hovered
                highlightMaterial->tint = glm::vec4(1.0f, 1.0f, 1.0f, 0.0f);
            }
